...
namespace MainBit.Projections.ClientSide.Controllers {
    ...
    public class LayoutController : Controller {
        ...
        public ActionResult AddDefaultClientSide(int id)
        {
            ...
            var typeDescriptors = _projectionManager.DescribeLayouts().ToList();

            for (var i = typeDescriptors.Count - 1; i >= 0; i--)
            {
                var typeDescriptor = typeDescriptors[i];
                typeDescriptor.Descriptors = typeDescriptor.Descriptors.Where(d => _clientSideLayoutEditorSelector.GetEditor(d.Form) != null);
            ...
            var viewModel = new LayoutAddViewModel { Id = id, Layouts = typeDescriptors };
            return View(viewModel);
        }
        ...
        public ActionResult CreateDefaultClientSide(int id, string category, string type)
        {
            if (!Services.Authorizer.Authorize(Permissions.ManageQueries, T("Not authorized to manage queries")))
                return new HttpUnauthorizedResult();
            ...
            var clientSidelayoutEditor = _clientSideLayoutEditorSelector.GetEditor(layout.Form);

            if (clientSidelayoutEditor == null)
            {
                return HttpNotFound();
            }

            var defaultState = clientSidelayoutEditor.BuildDefaultState(layout, _formManager);
            ...
            var layoutRecord = new LayoutRecord {
                ...
                Display = Convert.ToInt32(defaultState["Display"]),
                DisplayType = defaultState["DisplayType"]
            };
