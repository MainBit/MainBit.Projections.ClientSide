...
namespace MainBit.Projections.ClientSide.Controllers
{
    ...
    public class SortCriterionController : Controller {
        public SortCriterionController(
            ...
            IClientSideSortEditorSelector clientSideSortEditorSelector
            ) {
            Services = services;
            ...
            Shape = shapeFactory;
            _clientSideSortEditorSelector = clientSideSortEditorSelector;
        }
        ...
        private readonly IClientSideSortEditorSelector _clientSideSortEditorSelector;
        ...
        public ActionResult AddDefaultClientSide(int id)
        {
            ...
            var typeDescriptors = _projectionManager.DescribeSortCriteria().ToList();

            for (var i = typeDescriptors.Count - 1; i >= 0; i--)
            {
                var typeDescriptor = typeDescriptors[i];
                typeDescriptor.Descriptors = typeDescriptor.Descriptors.Where(d => _clientSideSortEditorSelector.GetEditor(d.Form) != null);
            ...
            var viewModel = new SortCriterionAddViewModel { Id = id, SortCriteria = typeDescriptors };
            return View(viewModel);
        }
        ...
        public ActionResult EditDefaultClientSide(int id, string category, string type)
        {
            ...
            var clientSideSortEditor = _clientSideSortEditorSelector.GetEditor(sortCriterion.Form);

            if (clientSideSortEditor == null)
            {
                return HttpNotFound();
            }

            var defaultState = clientSideSortEditor.BuildDefaultState(sortCriterion);
