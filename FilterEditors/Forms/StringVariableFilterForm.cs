...
namespace MainBit.Projections.ClientSide.FilterEditors.Forms {
    ...
    public class StringVariableFilterForm : IFormProvider
    {
        public const string FormName = "StringVariableFilter";
        public const string Separator = ";";

        protected dynamic Shape { get; set; }
        public Localizer T { get; set; }

        public StringVariableFilterForm(IShapeFactory shapeFactory) {
            Shape = shapeFactory;
        ...
        public void Describe(DescribeContext context) {
            Func<IShapeFactory, object> form =
                shape => {
                    ...
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.Equals), Text = T("Is equal to").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.NotEquals), Text = T("Is not equal to").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.Contains), Text = T("Contains").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.ContainsAny), Text = T("Contains any word").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.ContainsAll), Text = T("Contains all words").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.Starts), Text = T("Starts with").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.NotStarts), Text = T("Does not start with").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.Ends), Text = T("Ends with").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.NotEnds), Text = T("Does not end with").Text });
                    f._Fs1._Operator.Add(new SelectListItem { Value = Convert.ToString(StringOperator.NotContains), Text = T("Does not contain").Text });

                    f._Fs3._OperatorUndefined.Add(new SelectListItem { Value = Convert.ToString(StringUndefinedOperator.Any), Text = T("Any").Text });
                    f._Fs3._OperatorUndefined.Add(new SelectListItem { Value = Convert.ToString(StringUndefinedOperator.Undefined), Text = T("Undefined").Text });
        ...
        public static Action<IHqlExpressionFactory> GetFilterPredicate(dynamic formState, string property)
        {
            ...
            var arSeparetor = new string[] { Separator };
            ...
            if (string.IsNullOrEmpty((string)value))
            {
                switch (opUndef)
                {
                    case StringUndefinedOperator.Any:
                        return null; //return x => x.Or(x1 => x1.IsNotNull(property), x2 => x2.IsNull(property)); throw new NotNeedApplyFilterException()
            ...
            switch (op)
            {
                ...
                case StringOperator.ContainsAny:
                    var values1 = Convert.ToString(value).Split(arSeparetor, StringSplitOptions.RemoveEmptyEntries).Select(p => p.Trim()).ToArray();
                ...
                case StringOperator.ContainsAll:
                    var values2 = Convert.ToString(value).Split(arSeparetor, StringSplitOptions.RemoveEmptyEntries).Select(p => p.Trim()).ToArray();
